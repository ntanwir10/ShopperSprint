 ...
 * Update user profile
 */
router.put("/profile", authMiddleware.requireAuth, profileUpdateValidation, async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
 ...
 */
router.put("/change-password", authMiddleware.requireAuth, passwordChangeValidation, async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
 ...
 */
router.get("/verify", authMiddleware.requireAuth, async (req: Request, res: Response) => {
  try {
    res.json({
